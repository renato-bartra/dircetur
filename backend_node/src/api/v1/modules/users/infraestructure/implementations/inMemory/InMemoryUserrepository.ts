...
export class InMemoryUserRepository implements UserRepository {
  ...
  getById = async (id: number): Promise<User | null| string> => {
    const userFound: User|undefined = this.userData.find(x => x.id === id);
    if (userFound === undefined) return null;
    return userFound;
  };
  // Actualiza un usuario
  update = async (id: number, user: User): Promise<User|null|string> => {
    let userUpdate: User|undefined = this.userData.find(x => x.id === id);
  ...
  delete = async (id: number): Promise<boolean> => {
    const userDeleted = this.userData.find(x => x.id !== id)!;
