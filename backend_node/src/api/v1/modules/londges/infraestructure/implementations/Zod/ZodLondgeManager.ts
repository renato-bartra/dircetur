...
export class ZodLondgeValidator implements ValidatorManager {
  ...
  validate = async (user: object): Promise<object | null> => {
    ...
    const userSchema: z.ZodSchema<Londge> = z.strictObject({
      id: z.number(),
      city_id: z.number(),
      clase_id: z.number(),
      password: z.string(),
      email: z.email({message:"El email no tiene el formato requedido, ejmp: ejemplo@ejemplo.com"}),
      ...
      reservation_email: z.email({message:"El email para reservaciónes no tiene el formato requedido, ejmp: ejemplo@ejemplo.com"}).nullable(),
    });
    // Validate
    try {
