...
export class MySQLDepartmentRespository implements DepartmentRepository {
  ...
  getAll = async (): Promise<Department[]> => {
    ...
    const [rows]: [Department[]] = await promiseConn
      .promise()
      .execute('CALL sp_departments_get_all()')
      .then(([rows, fields]) => {
  ...
  getById = async (id: number): Promise<Department | null> => {
    ...
    const [rows, error]: [Department[], Error] = await promiseConn
      .promise()
      .execute('CALL sp_departments_get_by_id(?);', [id])
      .then(([rows, fields]) => {
        this.errorValidate = false;
        return rows;
      })
